...
unsafe fn bind_all_raw(
    ...
    for (i, v) in values.iter().enumerate() {
        ...
        let rc = match v {
            ...
            // Mirror core::db::Statement::bind: WitValue's typed
            // identity doesn't survive the SQL-layer flatten, so
            // bind the canonical-CBOR payload as a BLOB.
            db::Value::WitValue(p) => ffi::sqlite3_bind_blob(
                stmt,
                idx,
...
fn db_to_hl_value(v: db::Value) -> HlValue {
    match v {
        ...
        // The high-level surface (sqlite:wasm/high-level) has no
...
mod host_scalars {
    ...
    use crate::db;
    use bindings::exports::sqlite::wasm::dispatch_bridge::Guest as DispatchBridgeGuest;
    use bindings::exports::sqlite::wasm::dispatch_bridge_cas::Guest as DispatchBridgeCasGuest;
    use bindings::exports::sqlite::extension::spi::{
    ...
    pub(super) fn register_host_scalar(
        ...
        let callback = move |args: &[db::Value]| -> Result<db::Value, db::Error> {
            ...
            match bindings::sqlite::wasm::dispatch::scalar_call(
                &ext_name_cb,
                func_id,
                &imp_args,
            ) {
    ...
    impl db::Aggregate<u64> for HostAggregate {
        ...
        fn step(&self, ctx: &mut u64, args: &[db::Value]) -> Result<(), db::Error> {
            let imp_args: Vec<ImpSqlValue> =
                args.iter().cloned().map(db_to_imp_value).collect();
            bindings::sqlite::wasm::dispatch::aggregate_step(
                &self.ext_name,
                self.func_id,
                *ctx,
                &imp_args,
        ...
        fn finalize(&self, ctx: Option<u64>) -> Result<db::Value, db::Error> {
            ...
            bindings::sqlite::wasm::dispatch::aggregate_finalize(
                &self.ext_name,
                self.func_id,
                ctx_id,
            )
    ...
    impl db::WindowAggregate<u64> for HostAggregate {
        fn value(&self, ctx: &u64) -> Result<db::Value, db::Error> {
            bindings::sqlite::wasm::dispatch::aggregate_value(
                &self.ext_name,
                self.func_id,
                *ctx,
            )
        ...
        fn inverse(&self, ctx: &mut u64, args: &[db::Value]) -> Result<(), db::Error> {
            let imp_args: Vec<ImpSqlValue> =
                args.iter().cloned().map(db_to_imp_value).collect();
            bindings::sqlite::wasm::dispatch::aggregate_inverse(
                &self.ext_name,
                self.func_id,
                *ctx,
                &imp_args,
    ...
    pub(super) fn register_host_collation(
        ...
        let compare = move |a: &str, b: &str| -> std::cmp::Ordering {
            let rc =
                bindings::sqlite::wasm::dispatch::collation_compare(&ext_name_cb, collation_id, a, b);
            if rc < 0 {
    ...
    pub(super) fn register_host_update_hook(
        ...
        let callback = move |action: db::UpdateAction, db: &str, table: &str, rowid: i64| {
            bindings::sqlite::wasm::dispatch::on_update(
                &ext_name_cb,
    ...
    pub(super) fn register_host_commit_hook(
        ...
        let callback = move || -> bool {
            let allow = bindings::sqlite::wasm::dispatch::on_commit(&ext_name_cb);
            !allow
        };
    ...
    pub(super) fn register_host_rollback_hook(
        ...
        let ext_name_cb = ext_name.clone();
        let callback = move || {
            bindings::sqlite::wasm::dispatch::on_rollback(&ext_name_cb);
        };
    ...
    pub(super) fn register_host_wal_hook(
        ...
        let callback = move |db_name: &str, n_frames: i32| -> i32 {
            ...
            bindings::sqlite::wasm::dispatch::wal_hook(
                &ext_name_cb,
                hook_id,
                db_name,
                frames_u32,
